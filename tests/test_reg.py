...
from gconanpy.reg import Abbreviator, DunderParser, Regextract
from gconanpy.testers import LIPSUM, Tester
...
class TestRegextract(Tester):
    ...
    def test_iter_parentheticals(self) -> None:
        for parenthetical in Regextract.iter_parentheticals(LIPSUM):
            pass  # TODO FIX iter_parenthetials!


class TestAbbreviator(Tester):
    def test_abbreviate_shorten(self) -> None:
        self.check_result(Abbreviator.abbreviate(
            "HelloWorld", 5, **{"Hello": "H"}), "HWorld")
        self.check_result(Abbreviator.abbreviate(
            "HelloWorld", 5, **{"Hello": "H", "World": "W"}), "HW")

    def test_abbreviate_long(self) -> None:
        self.check_result(Abbreviator.abbreviate(
            "HelloWorld", 10, **{"Hello": "H"}), "HelloWorld")
        self.check_result(Abbreviator.abbreviate(
            "HelloWorld", 10, **{"Hello": "H", "World": "W"}), "HelloWorld")
